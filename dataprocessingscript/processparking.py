...
df_parkhour['date'] = df_parkhour['date'].dt.strftime('%Y-%m-%d')
...
df_parkday['date'] = df_parkday['date'].dt.strftime('%Y-%m-%d')
#print(df_parkday)
...
# top3 utilization
top3util=pd.DataFrame(df_merge.groupby(['date','icao_code'],as_index=False).agg({'count_of_occupancy':('sum'),'count_of_slots':pd.Series.nunique}))
